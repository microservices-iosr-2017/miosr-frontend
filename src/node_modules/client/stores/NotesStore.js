

class NotesStore {
  constructor(notesActions, notesSource) {
    this.bindListeners({
      onFetch: notesActions.FETCH,
      onFetchSucceess: notesActions.FETCH_SUCCESS,
      onFetchFailure: notesActions.FETCH_FAILURE,
      onSelect: notesActions.SELECT,
      onDeselect: notesActions.DESELECT,
      onAdd: notesActions.ADD,
    });

    this._source = notesSource;
    this.notesList = []; // list of objects as received from server
    this.selectedNote = null; // object representing current selection
    this.selectionEditable = false;
    this.error = "no error";
  }

  onFetch() {
    this._source.fetch();
  }

  onFetchSucceess(payload) {
    this.setState({notesList: payload, selectedNote: null});
  }

  onFetchFailure(err) {
    this.setState({error: JSON.stringify(err)});
  }

  onSelect(id) {
    if(this.selectedNote) this.onDeselect();
    this.setState({selectedNote: this.notesList[id], selectionEditable: false});
  }

  onDeselect() {
    this.setState({selectedNote: null});
  }

  onAdd() {
    let newState = {};
    if(this.selectionEditable) {
      newState.notesList = this.notesList.concat([this.selectedNote])
    } else {
      newState.notesList = this.notesList;
    }

    newState.selectedNote = {title: 'New notes', text: ''};
    newState.notesList.push(newState.selectedNote);
    newState.selectionEditable = true;

    this.setState(newState);
  }
}

export default function build({altInstance, notesActions, notesSource}) {
  return altInstance.createStore(NotesStore, 'NotesStore', notesActions, notesSource);
}
